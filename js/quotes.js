const quotes = [
  ...
  const innerquotes = document.querySelector(".quote #quoteContent");
  const randomQuetes = Math.floor(Math.random() * quotes.length);
  
  const quote = quotes[randomQuetes].quote;
  const author = quotes[randomQuetes].author;
  
  innerquotes.innerHTML = `오늘의 명언<br/> ${quote}<br/> -${author}-`;
